...
-- Version: 20260909142512  Name: restore_kds_legacy_compat_with_origin_rate_limit
...
begin
  ...
  if v_claim_role = 'anon' then
    ...
    if v_origin not in (
      ...
      'https://rohmat-kds-printer-gramatikafoundation-3204.vercel.app'
    ) then
    ...
    if coalesce((v_ip_rl->>'allowed')::boolean,false) is not true then
      return jsonb_build_object('ok',false,'error','rate_limited','retry_after',coalesce((v_ip_rl->>'retry_after')::int,900));
    end if;
  end if;
  ...
  if coalesce((v_rl->>'allowed')::boolean,false) is not true then
    return jsonb_build_object('ok',false,'error','rate_limited','retry_after',coalesce((v_rl->>'retry_after')::int,900));
  end if;
  ...
  where au.email=lower(trim(p_email))::extensions.citext and au.is_active
  limit 1;
  ...
  if v_claim_role='anon' and v_ip_key is not null then
    delete from public.security_rate_limits where bucket='kds-legacy-login-ip' and key_hash=v_ip_key;
  end if;
  ...
  values(encode(extensions.digest(v_token,'sha256'),'hex'),v_email,v_exp);
  ...
  where s.email=v_email
    and s.token_hash not in (
      ...
      where s2.email=v_email order by s2.expires_at desc limit 3
    );
...
revoke execute on function public.admin_password_login(text,text) from public, authenticated;
revoke execute on function public.admin_password_session_info(text) from public, authenticated;
revoke execute on function public.admin_password_logout(text) from public, authenticated;
revoke execute on function public.kds_snapshot(text) from public, authenticated;
revoke execute on function public.kds_update_order(text,uuid,text) from public, authenticated;
revoke execute on function public.kds_set_availability(text,text,boolean,text) from public, authenticated;
