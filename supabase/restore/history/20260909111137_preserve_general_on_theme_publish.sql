...
-- Version: 20260909111137  Name: preserve_general_on_theme_publish
...
as $$
declare
  v_sys jsonb; v_old_sys jsonb; v_tokens jsonb; v_gen_old jsonb; v_gen jsonb; v_theme jsonb; v_payload jsonb; v_colors jsonb; v_gc jsonb; v_typo jsonb; v_layout jsonb; v_theme_id text; v_theme_name text; v_id bigint; v_changed boolean; v_draft jsonb; v_theme_changed boolean; v_eff jsonb; v_ec jsonb; v_et jsonb; v_el jsonb;
begin
  ...
  if new.admin_design is distinct from old.admin_design and (v_sys-'draft')=(v_old_sys-'draft') and coalesce(v_sys->'draft','null'::jsonb) is distinct from coalesce(v_old_sys->'draft','null'::jsonb) then
    v_draft:=v_sys->'draft';
    if v_draft is null or v_draft='null'::jsonb then new.design_system:=jsonb_set(coalesce(old.design_system,'{}'::jsonb),'{draft}','null'::jsonb,true); return new; end if;
    v_tokens:=coalesce(v_draft->'tokens','{}'::jsonb);
    if jsonb_typeof(v_tokens->'colors')='array' then v_colors:=jsonb_build_object('background',v_tokens#>>'{colors,0}','panel',v_tokens#>>'{colors,1}','primary',v_tokens#>>'{colors,2}','accent',v_tokens#>>'{colors,3}','text',v_tokens#>>'{colors,4}','muted',v_tokens#>>'{colors,5}'); else v_colors:='{}'::jsonb; end if;
    v_theme_id:=coalesce(v_draft->>'themeId',v_tokens->>'id','custom'); v_theme_name:=coalesce(v_draft->>'themeName',v_tokens->>'name','Custom Theme');
    ...
    new.design_system:=coalesce(old.design_system,'{}'::jsonb)||jsonb_build_object('version',2,'draft',v_payload||jsonb_build_object('status','draft','draftVersionId',v_id,'updatedAt',now()),'updatedAt',now()); return new;
  end if;

  v_changed:=new.admin_design is distinct from old.admin_design or new.color_outer is distinct from old.color_outer or new.color_panel is distinct from old.color_panel or new.color_primary is distinct from old.color_primary or new.color_accent is distinct from old.color_accent or new.color_text is distinct from old.color_text or new.color_muted is distinct from old.color_muted or new.content_width is distinct from old.content_width or new.typography is distinct from old.typography;
  if not v_changed then return new; end if;

  v_theme_changed:=coalesce(v_sys->>'activeTheme','') is distinct from coalesce(v_old_sys->>'activeTheme','') or coalesce(v_sys->'published','{}'::jsonb) is distinct from coalesce(v_old_sys->'published','{}'::jsonb);
  v_tokens:=coalesce(v_sys#>'{published,tokens}','{}'::jsonb); v_gen_old:=coalesce(v_sys->'general','{}'::jsonb);
  ...
  if jsonb_typeof(v_tokens->'colors')='array' then v_colors:=jsonb_build_object('background',coalesce(v_tokens#>>'{colors,0}',new.color_outer),'panel',coalesce(v_tokens#>>'{colors,1}',new.color_panel),'primary',coalesce(v_tokens#>>'{colors,2}',new.color_primary),'accent',coalesce(v_tokens#>>'{colors,3}',new.color_accent),'text',coalesce(v_tokens#>>'{colors,4}',new.color_text),'muted',coalesce(v_tokens#>>'{colors,5}',new.color_muted)); else v_colors:=jsonb_build_object('background',new.color_outer,'panel',new.color_panel,'primary',new.color_primary,'accent',new.color_accent,'text',new.color_text,'muted',new.color_muted); end if;
  ...
  if v_theme_changed and jsonb_typeof(old.design_system#>'{published,general}')='object' then
  ...
  if (v_ec->>'background')~'^#[0-9A-Fa-f]{6}$' then new.color_outer:=v_ec->>'background'; end if; if (v_ec->>'panel')~'^#[0-9A-Fa-f]{6}$' then new.color_panel:=v_ec->>'panel'; end if; if (v_ec->>'primary')~'^#[0-9A-Fa-f]{6}$' then new.color_primary:=v_ec->>'primary'; end if; if (v_ec->>'accent')~'^#[0-9A-Fa-f]{6}$' then new.color_accent:=v_ec->>'accent'; end if; if (v_ec->>'text')~'^#[0-9A-Fa-f]{6}$' then new.color_text:=v_ec->>'text'; end if; if (v_ec->>'muted')~'^#[0-9A-Fa-f]{6}$' then new.color_muted:=v_ec->>'muted'; end if;
  ...
  new.design_system:=jsonb_build_object('version',2,'published',v_payload||jsonb_build_object('status','published','publishedAt',now(),'publishedBy','legacy-admin-bridge'),'draft',old.design_system->'draft','publishedVersionId',v_id,'updatedAt',now()); return new;
end
$$;
