...
-- Version: 20260912181923  Name: kds_mobile_stable_fingerprint_v2
-- Production-specific credentials, operator identities, and project endpoint were neutralized.

create or replace function private.request_device_fingerprint_hash()
returns text
language plpgsql
stable
set search_path=''
...
begin
  ...
  v_lang := left(coalesce(h->>'accept-language','unknown'),120);
  v_platform := left(coalesce(h->>'sec-ch-ua-platform','unknown'),80);
  return encode(extensions.digest(v_ua||'|'||v_lang||'|'||v_platform,'sha256'),'hex');
end
$function$;
...
as $function$
  ...
  where s.token_hash=encode(extensions.digest(coalesce(p_token,''),'sha256'),'hex')
    ...
    and (
      ...
      or s.client_fingerprint_hash = private.request_device_fingerprint_hash()
    )
  limit 1
$function$;
...
set client_fingerprint_hash = null
where session_scope='kds' and expires_at>now();
