...
comment on function public.master_prototype_runtime_context(uuid,text,text) is
  'Service-role runtime resolver. Under final RLS enforcement only the legacy reference KDS bridge may resolve without tenant/origin; Public/Admin remain fail-closed and all tenant-aware clients use explicit tenant or registered origin.';
