...
-- Version: 20260908163659  Name: format_kds_payment_print_lines
  ...
  select coalesce(
    jsonb_agg(
      to_jsonb(o) || jsonb_build_object(
        'producer_note',
        v_break ||
        coalesce(nullif(trim(o.producer_note),''),'Nominal pembayaran sesuai dengan total pesanan.') || v_break ||
        ...
        '| Waktu pembayaran: ' || coalesce(
          ...
          '—'
        ) || case when coalesce(nullif(trim(o.proof_time_detected),''), case when o.payment_submitted_at is not null then to_char(o.payment_submitted_at at time zone 'Asia/Jakarta','HH24:MI') end) is not null then ' WIB' else '' end || v_break ||
